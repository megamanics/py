...
w={}
for file_name in text_files:
    value = file_contents(file_name)
    for line in value.splitlines():
        word = line.split(':::')
        for author in word[1].split('::'):
            author = author.lower()
            author = author.replace('.','')
            for term in word[2].split():
                term = term.lower()
                term = term.replace('.','')
                term = term.replace('<i>','')
                term = term.replace('</i>','')
...
for (k,v) in sorted(w.items(),key=itemgetter(1)):
    if (str(k).find('alan m frieze') > 0):
        if (v > 3):
            print k,v

for (k,v) in sorted(w.items(),key=itemgetter(1)):
    if (str(k).find('michael stonebraker') > 0):
        if (v > 3):
            print k,v        

for (k,v) in sorted(w.items(),key=itemgetter(1)):
    if (str(k).find('sudhakar m reddy') > 0):
        if (v > 3):
            print k,v          

for (k,v) in sorted(w.items(),key=itemgetter(1)):
    if (str(k).find('zdzislaw pawlak') > 0):
        if (v > 3):
            print k,v          

#f = open('big_outfile','w')
#f.write(str(w))
#f.close()

end = time.time()
print 'Complete in %s' %(end-start)