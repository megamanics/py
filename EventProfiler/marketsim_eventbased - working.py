...
def getEvents(dt_end, dt_start,symbollist):
    '''Get Event DataFrame'''
    ...
    df_events = find_events2(ls_symbols, d_data)
...
def find_events2(ls_symbols, d_data):
    ...
    df_yesterday = df_close >= 5.0
    df_today = df_close < 5.0
...
def analyzeTransactions(investment,df_trans,ldf_dataframe,benchmark):    
    ...
    for trans_dt in df_trans.index:
        for sym in df_trans:      
...
if __name__ == '__main__':
    ...
    (df_data,df_events) = getEvents(dt_end, dt_start, "sp5002012")
