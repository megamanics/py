...
def genetic_annealing(starter_list):
    ...
    for temperature in np.arange(3.0, 0.0, -0.06):
        ...
        for portfolio in population:
            ...
            fitness.append( (sharpe, mean_daily_return*252.,) ) #Added in annualized return -- fortunately this doesn't break the sort below...
        # Remove unfit members of population
        temp_tuple = zip(population, fitness)
        temp_tuple = [t for t in temp_tuple if t[1][1] >= 0.4 ]
        temp_tuple = sorted(temp_tuple, key=itemgetter(1))
        temp_tuple = temp_tuple[-n:]
        a, b = zip(*temp_tuple)
        population = list(a)
        print "Temp: %4.2f  Sharpe:  %13.11f  AnnualRet: %4.2f%%  Symbols: %30s Allocations: %30s" % (temperature, b[-1][0], b[-1][1]*100., a[-1].symbols, a[-1].allocations)
...
def generate_random_portfolios(num_portfolios, stock_symbols, portfolio_size):
    ...
    for i in range(num_portfolios):
        ...
        alloc = alloc/sum(alloc)  #Normalize, probably not necessary?
