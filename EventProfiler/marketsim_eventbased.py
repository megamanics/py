...
def get_bollinger_events(dt_start,dt_end,symbollist,iteration):
    ''' Event defined by bollinger band:
    Bollinger value for the equity today <= -2.0
    Bollinger value for the equity yesterday >= -2.0
    Bollinger value for SPY today >= 1.0 278 events
    Bollinger value for SPY today >= 1.5 107 events
    Bollinger value for SPY today >= 1.4 107 events'''
    ...
    std   = pd.rolling_std(df_close,20,min_periods=20)
    bollinger_val       = (df_close - means)/std
    
    bollinger_yesterday = bollinger_val.shift(1)
    bollinger_yesterday.fillna(0,inplace=True)
    
    df_condition1       = bollinger_val        <= -2.0
    df_condition2       = bollinger_yesterday  >= -2.0
    df_condition3       = bollinger_val['SPY'] >= 1.2
    
    df_events = copy.deepcopy(df_close)
    df_events = df_events * np.NAN
    
    df_cond12  = df_condition1        & df_condition2 
    for sym in df_events:
        for t in df_events[sym].index:
                if (df_cond12.ix[t][sym] & df_condition3.ix[t]):
                    df_events.ix[t][sym] = 1                
    #df_bool    = df_cond12.values     & df_condition3[:,np.newaxis]
    #df_events[df_bool] = 1
...
def comparePortfolios(ls_portfolio):
    ...
    for portlabel in ls_portfolio:
        ...
        #plt.plot_date(na_normalized_price.index, na_normalized_price, fmt='bo', xdate=True, ydate=False)  
...
def priceDropEventStudy():
    ...
    Portfolio Name                    =  PriceDrop10
    Final Value of the portfolio      =  59789.0
    Sharpe Ratio of Fund              =  0.606442727929
    Total Return of Fund              =  1.19578
    Standard Deviation of Fund        =  0.0108230451682
    Average Daily Return of Fund      =  0.000413465229381
    Portfolio Name                    =  PriceDrop8
    Final Value of the portfolio      =  65147.0
    Sharpe Ratio of Fund              =  1.00430543785
    Total Return of Fund              =  1.30294
    Standard Deviation of Fund        =  0.00892281323007
    Average Daily Return of Fund      =  0.000564504419492
    Portfolio Name                    =  PriceDrop9
    Final Value of the portfolio      =  61990.0
    Sharpe Ratio of Fund              =  0.747127901246
    Total Return of Fund              =  1.2398
    Standard Deviation of Fund        =  0.0101492389959
    Average Daily Return of Fund      =  0.000477670217813
    Portfolio Name                    =  PriceDrop6
    Final Value of the portfolio      =  57954.0
    Sharpe Ratio of Fund              =  0.80680084008
    Total Return of Fund              =  1.15908
    Standard Deviation of Fund        =  0.00613073390739
    Average Daily Return of Fund      =  0.000311586432059
    Portfolio Name                    =  PriceDrop7
    Final Value of the portfolio      =  60309.0
    Sharpe Ratio of Fund              =  0.844573106455
    Total Return of Fund              =  1.20618
    Standard Deviation of Fund        =  0.00751913991054
    Average Daily Return of Fund      =  0.000400041589041
    Portfolio Name                    =  PriceDrop5
    Final Value of the portfolio      =  54824.0
    Sharpe Ratio of Fund              =  0.526290122869
    Total Return of Fund              =  1.09648
    Standard Deviation of Fund        =  0.00606729722357
    Average Daily Return of Fund      =  0.000201150084661
'''  
    ...
    for i in range(5,11,1):
        portfolioLabel = "PriceDrop" + str(i)
        eventfilename  = portfolioLabel + ".pdf"
        ls_portfolioLabel.append(portfolioLabel)    
        (df_data,df_events) = get_price_events(dt_start,dt_end,"sp5002012",float(i))
...
def PlotBBIndicator():
    ...
    2010-12-30 16:00:00  0.793493  0.634661  1.959324  0.498395
                           ...
                           AAPL      GOOG       IBM      MSFT       SPY
    2010-05-12 16:00:00  0.515937 -1.051277  1.770595 -0.958810 -0.401870
    2010-05-21 16:00:00 -1.376452 -2.105740 -0.963326 -1.990359 -1.662943
    
    Minor differences in float values may arise due to different implementations.
    '''  
    symbols = ["AAPL","GOOG","IBM","MSFT"]
    pd_data = getYahooData(dt.datetime(2010,1,1), dt.datetime(2010,12,31), symbols)
    pd_close = pd_data['close']
    means = pd.rolling_mean(pd_close,20,min_periods=20)
    std   = pd.rolling_std(pd_close,20,min_periods=20)
    bblower = pd_close - std
    bbupper = pd_close + std
    bollinger_val = (pd_close - means)/std
    print bollinger_val.ix[dt.datetime(2010,5,12,16)]
    print bollinger_val.ix[dt.datetime(2010,5,21,16)]
    print bollinger_val.ix[dt.datetime(2010,6,14,16)]
    print bollinger_val.ix[dt.datetime(2010,12,23,16) :]
    # Plot the prices
    plt.clf()
    symtoplot = 'AAPL'
    plt.plot(pd_close.index,pd_close[symtoplot].values,label=symtoplot)
    plt.plot(pd_close.index,means[symtoplot].values)
    plt.plot(pd_close.index,bblower[symtoplot].values)
    plt.plot(pd_close.index,bbupper[symtoplot].values)
    plt.legend([symtoplot,'Moving Avg','BB Lower','BB Upper'])
    plt.ylabel('Adjusted Close')
    fileName = "BollingerBands.pdf"
    plt.savefig(fileName, format='pdf') 
...
if __name__ == '__main__':
    ...
    #priceDropEventStudy()
    #BBEventStudy()
    #eventStudy("PriceDrop", dt.datetime(2008, 1, 1), dt.datetime(2009, 12, 31), get_price_events, "sp5002012")
    eventStudy("BBEvent", dt.datetime(2008, 1, 1), dt.datetime(2009, 12, 31), get_bollinger_events, "sp5002012")
    end = ti.time()
